...
**Entscheidung: beide Änderungen (Lock-Verfeinerung UND Queue-Deckel) vollständig zurückgesetzt.** Der aktuell ausgelieferte Stand bleibt exakt der bereits verifizierte 100ms/2.000 mit vollem `cs.mu.Lock()` in `flushWALBatch` (Runde 2) — nicht wegen eines Fehlers darin, sondern weil die Alternative in dieser Untersuchung zwei neue, echte Probleme aufwarf, die selbst weitere eigene Untersuchung/Härtung bräuchten (u. a. eine deterministische Zeilen-Sperrreihenfolge zwischen Flush-Worker- und Batcher-UPSERT, und ein sanfterer Backpressure-Mechanismus statt einer harten Kante bei einem festen Queue-Deckel), bevor irgendetwas davon auch nur in Erwägung gezogen werden sollte, geschweige denn an ein laufendes, geldbewegendes Live-Ledger.

**Wichtige, eigenständige Erkenntnis aus dieser Runde, unabhängig vom Ausgang:** der entdeckte Postgres-Deadlock zwischen Flush-Worker- und Batcher-Schreibpfad ist ein LATENTES Risiko, das vermutlich (in abgeschwächter Form) bereits im aktuell ausgelieferten Code existiert, sobald WAL aktiv ist UND genug gleichzeitiger Verkehr eine Mischung aus Fastpath-Fallback (Batcher) und WAL-Flush gleichzeitig erzeugt — diese Untersuchung hat es nur durch künstlich verstärkten Batcher-Anteil sichtbar gemacht, nicht erzeugt. Für eine künftige Härtung: Flush-Worker und Batcher sollten Zeilen in derselben deterministischen Reihenfolge sperren (z. B. beide nach Adresse sortiert, wie `LockAddrs` es bereits für In-Memory-Shard-Locks tut), um dieses Deadlock-Muster grundsätzlich auszuschließen, statt sich auf Postgres' eigene Deadlock-Erkennung (die einen der beiden Transaktionen einfach abbricht) als Sicherheitsnetz zu verlassen.
