...
Wegen Befund (1) wurde WAL sofort wieder deaktiviert und über einen Verify-Lauf bestätigt: null `[WAL]`-Zeilen, `ENABLE_MULTI_BLOCK_TICK`-Warnung weg, Höhe steigt normal weiter. Contabo2 läuft wieder auf dem alten, synchronen Pfad — Punkt 1 oben ("nichts liest oder schreibt das WAL im echten Betrieb") gilt am Ende dieser Session also wieder uneingeschränkt, die Ledger-Durability-Semantik ist unverändert. Das Deploy-Tooling ist jetzt korrekt (Datei-Flags werden endlich zuverlässig gesetzt UND entfernt), aber **eine erneute Aktivierung braucht zuerst einen Persistenz-Mechanismus für die WAL-Datei, der einen `docker rm` übersteht** (Volume-Mount oder gleichwertig) — sonst bleibt das Datenverlustrisiko trotz funktionierendem Tooling bestehen. Details, exakte Patch-Historie und der Nebenbefund zu vermehrten Orphan/Reject-Log-Zeilen während `ENABLE_MULTI_BLOCK_TICK=1` aktiv war: siehe STAGING_RUNBOOK.md.
