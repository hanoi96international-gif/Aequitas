...
**Entscheidung:** zurückgesetzt auf die ursprünglichen, bereits getesteten Werte (500/500ms), NICHT auf einen der drei Versuche. Richtiges Tuning braucht eine deutlich längere (Minuten, nicht Sekunden) Sustained-Load-Messung, die die Flush-Queue ihren eigenen echten Steady-State erreichen lässt — im Idealfall Teil der ohnehin geplanten mehrtägigen Staging-Kampagne (STAGING_RUNBOOK.md), nicht ein Fix aus einer einzelnen Chat-Session gegen ein laufendes, geldbewegendes Live-Ledger. Das unbegrenzte-Wachstum-Risiko bei echtem Sustained-Verkehr ist real und hiermit dokumentiert; der Fix ist es bewusst nicht.
