...
**Update (2026-07-23) — Live-Aktivierungsversuch auf Contabo2, Ergebnis: nie tatsächlich angekommen.** Entgegen der Warnung oben wurde `AEQUITAS_WAL_ENABLED`/`ENABLE_MULTI_BLOCK_TICK` bereits einmal live auf Contabo2 gesetzt (`enable-wal-contabo2.yml`), vor der eigentlich vorgeschriebenen Staging-Kampagne. Root Cause jetzt bestätigt (siehe STAGING_RUNBOOK.md, "Update — Contabo2-Live-Versuch"): `deploy_safe_c2.sh` übernimmt Env-Variablen für einen neuen Container ausschließlich aus `docker inspect` des vorherigen Containers, nie aus `/root/.aequitas.env` — die von `enable-wal-contabo2.yml` gesetzten Flags haben den laufenden Prozess deshalb nie erreicht, bestätigt durch `docker inspect` des tatsächlich laufenden Containers (keine der drei Variablen im Prozess-Env) und durch komplette Abwesenheit jeder `[WAL]`-Log-Zeile über mehrere Neustarts hinweg. Konsequenz: Punkt 1 oben ("nichts liest oder schreibt das WAL im echten Betrieb") gilt für Contabo2 weiterhin uneingeschränkt — die Ledger-Durability-Semantik wurde nie tatsächlich verändert, kein zusätzliches Produktionsrisiko eingegangen. Der Bug liegt ausschließlich im Deploy-Tooling, nicht in der WAL-Implementierung selbst. Ein Fix wurde entworfen, aber bewusst nicht ungefragt live auf den Server geschrieben (`deploy_safe_c2.sh` liegt außerhalb der Versionskontrolle dieses Repos) — siehe STAGING_RUNBOOK.md für den vorgeschlagenen Patch.
