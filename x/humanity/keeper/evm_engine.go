...
// untested surface.
// newStateDBLocked is the context.Background()-calling wrapper kept for
...
func (e *EVMEngine) newStateDBLocked(onlyAddrs ...common.Address) (*state.StateDB, state.Database, error) {
	return e.newStateDBLockedCtx(context.Background(), onlyAddrs...)
}

func (e *EVMEngine) newStateDBLockedCtx(ctx context.Context, onlyAddrs ...common.Address) (*state.StateDB, state.Database, error) {
	if len(onlyAddrs) == 0 {
	...
	for _, acc := range e.chainState.getAccountsForAddressesLockedCtx(ctx, addrStrs) {
		addr := common.HexToAddress(acc.Address)
		sdb.SetBalance(addr, aeqToWei(acc.Balance.Float()))
	}
...
// CallContractLocked is the context.Background()-calling wrapper kept for
// callers not yet migrated to thread ctx explicitly — see dbExecCtx's
// comment for the migration this is part of.
func (e *EVMEngine) CallContractLocked(from, to common.Address, data []byte, value *big.Int) (ret []byte, err error) {
	return e.CallContractLockedCtx(context.Background(), from, to, data, value)
}

func (e *EVMEngine) CallContractLockedCtx(ctx context.Context, from, to common.Address, data []byte, value *big.Int) (ret []byte, err error) {
	if value != nil && value.Sign() > 0 {
	...
	sdb, _, err := e.newStateDBLockedCtx(ctx, onlyAddrs...)
	if err != nil {
