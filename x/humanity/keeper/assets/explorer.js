...
en:{
  ...
  'co-how-t':'Setting one up — three steps',
  'co-how-1':'<strong style="color:var(--text)">Start it.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;your-hostname&gt;</code> generates both keys on that machine with permissions 600, starts the container, and prints your public key. Nothing to copy in beforehand, nothing to send anywhere.',
  'co-how-2':'<strong style="color:var(--text)">Sign the authorization.</strong> Paste that public key into the button below and sign with a wallet that is a registered human. Nothing is spent and nothing is transferred — it signs a sentence.',
  'co-how-3':'<strong style="color:var(--text)">Send it to the nodes.</strong> The page hands you a line ending in <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;signature&gt;</code>. Run it on your coordinator\'s host. <strong style="color:var(--text)">Signing alone changes nothing</strong> — this command is what writes the authorization, and it goes to every node at once.',
...
de:{
  ...
  'co-how-t':'Einrichten — drei Schritte',
  'co-how-1':'<strong style="color:var(--text)">Starten.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;dein-hostname&gt;</code> erzeugt beide Schlüssel auf dieser Maschine mit Rechten 600, startet den Container und gibt deinen öffentlichen Schlüssel aus. Nichts vorher hineinkopieren, nichts irgendwohin schicken.',
  'co-how-2':'<strong style="color:var(--text)">Autorisierung unterschreiben.</strong> Diesen öffentlichen Schlüssel in den Knopf unten einfügen und mit einer Wallet unterschreiben, die ein registrierter Mensch ist. Es wird nichts ausgegeben und nichts überwiesen — unterschrieben wird ein Satz.',
  'co-how-3':'<strong style="color:var(--text)">An die Knoten senden.</strong> Die Seite gibt dir eine Zeile, die auf <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;signatur&gt;</code> endet. Die führst du auf dem Rechner deines Coordinators aus. <strong style="color:var(--text)">Unterschreiben allein ändert nichts</strong> — erst dieser Befehl schreibt die Autorisierung, und er geht an alle Knoten auf einmal.',
...
es:{
  ...
  'co-how-t':'Puesta en marcha: tres pasos',
  'co-how-1':'<strong style="color:var(--text)">Arráncalo.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;tu-hostname&gt;</code> genera ambas claves en esa máquina con permisos 600, arranca el contenedor e imprime tu clave pública. No hay que copiar nada antes ni enviar nada a ninguna parte.',
  'co-how-2':'<strong style="color:var(--text)">Firma la autorización.</strong> Pega esa clave pública en el botón de abajo y firma con una cartera que sea una persona registrada. No se gasta ni se transfiere nada: se firma una frase.',
  'co-how-3':'<strong style="color:var(--text)">Envíala a los nodos.</strong> La página te entrega una línea que termina en <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;firma&gt;</code>. Ejecútala en el servidor de tu coordinador. <strong style="color:var(--text)">Firmar por sí solo no cambia nada</strong>: es este comando el que escribe la autorización, y va a todos los nodos a la vez.',
...
ru:{
  ...
  'co-how-t':'Установка — три шага',
  'co-how-1':'<strong style="color:var(--text)">Запустите.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;ваш-хост&gt;</code> создаёт оба ключа на этой машине с правами 600, запускает контейнер и печатает ваш публичный ключ. Ничего не нужно копировать заранее и никуда отправлять.',
  'co-how-2':'<strong style="color:var(--text)">Подпишите авторизацию.</strong> Вставьте этот публичный ключ в кнопку ниже и подпишите кошельком, который является зарегистрированным человеком. Ничего не тратится и не переводится — подписывается предложение.',
  'co-how-3':'<strong style="color:var(--text)">Отправьте её узлам.</strong> Страница выдаст строку, оканчивающуюся на <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;подпись&gt;</code>. Выполните её на сервере вашего координатора. <strong style="color:var(--text)">Одна лишь подпись ничего не меняет</strong> — авторизацию записывает именно эта команда, и она уходит сразу на все узлы.',
...
zh:{
  ...
  'co-how-t':'搭建 —— 三个步骤',
  'co-how-1':'<strong style="color:var(--text)">启动它。</strong><code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;你的主机名&gt;</code> 会在该机器上以 600 权限生成两把密钥、启动容器，并打印出你的公钥。事先无需复制任何东西，也不必向任何地方发送。',
  'co-how-2':'<strong style="color:var(--text)">签署授权。</strong>把那把公钥粘贴到下面的按钮里，用一个已注册为人类的钱包签名。不花费也不转移任何东西——签的是一句话。',
  'co-how-3':'<strong style="color:var(--text)">发送给各节点。</strong>页面会给你一行以 <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;签名&gt;</code> 结尾的命令。在你的协调器主机上执行它。<strong style="color:var(--text)">仅仅签名不会改变任何事</strong>——写入授权的是这条命令，而且它一次发往所有节点。',
...
id:{
  ...
  'co-how-t':'Menyiapkannya — tiga langkah',
  'co-how-1':'<strong style="color:var(--text)">Jalankan.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;hostname-kamu&gt;</code> membangkitkan kedua kunci di mesin itu dengan izin 600, menjalankan kontainer, dan mencetak kunci publikmu. Tak ada yang perlu disalin lebih dulu, tak ada yang perlu dikirim ke mana pun.',
  'co-how-2':'<strong style="color:var(--text)">Tandatangani pengesahannya.</strong> Tempel kunci publik itu ke tombol di bawah dan tandatangani dengan dompet yang merupakan manusia terdaftar. Tak ada yang dibelanjakan dan tak ada yang dipindahkan — yang ditandatangani adalah sebuah kalimat.',
  'co-how-3':'<strong style="color:var(--text)">Kirim ke node.</strong> Halaman memberimu satu baris yang berakhir dengan <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;tandatangan&gt;</code>. Jalankan di host koordinatormu. <strong style="color:var(--text)">Menandatangani saja tidak mengubah apa pun</strong> — perintah inilah yang menuliskan pengesahan, dan ia menuju semua node sekaligus.',
...
it:{
  ...
  'co-how-t':'Metterlo in piedi — tre passaggi',
  'co-how-1':'<strong style="color:var(--text)">Avvialo.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;il-tuo-hostname&gt;</code> genera entrambe le chiavi su quella macchina con permessi 600, avvia il container e stampa la tua chiave pubblica. Niente da copiare prima, niente da inviare da nessuna parte.',
  'co-how-2':'<strong style="color:var(--text)">Firma l\'autorizzazione.</strong> Incolla quella chiave pubblica nel pulsante qui sotto e firma con un portafoglio che sia una persona registrata. Non si spende e non si trasferisce nulla: si firma una frase.',
  'co-how-3':'<strong style="color:var(--text)">Mandala ai nodi.</strong> La pagina ti consegna una riga che termina con <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;firma&gt;</code>. Eseguila sull\'host del tuo coordinatore. <strong style="color:var(--text)">Firmare da solo non cambia nulla</strong>: è questo comando a scrivere l\'autorizzazione, e va a tutti i nodi in una volta.',
...
tr:{
  ...
  'co-how-t':'Kurulum — üç adım',
  'co-how-1':'<strong style="color:var(--text)">Başlatın.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;sunucu-adınız&gt;</code> her iki anahtarı o makinede 600 izinleriyle üretir, kabı başlatır ve açık anahtarınızı yazdırır. Önceden kopyalanacak bir şey yok, hiçbir yere gönderilecek bir şey yok.',
  'co-how-2':'<strong style="color:var(--text)">Yetkilendirmeyi imzalayın.</strong> O açık anahtarı aşağıdaki düğmeye yapıştırın ve kayıtlı bir insan olan bir cüzdanla imzalayın. Hiçbir şey harcanmaz, hiçbir şey aktarılmaz — imzalanan bir cümledir.',
  'co-how-3':'<strong style="color:var(--text)">Düğümlere gönderin.</strong> Sayfa size <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;imza&gt;</code> ile biten bir satır verir. Onu koordinatörünüzün sunucusunda çalıştırın. <strong style="color:var(--text)">Yalnızca imzalamak hiçbir şeyi değiştirmez</strong> — yetkilendirmeyi yazan bu komuttur ve tüm düğümlere aynı anda gider.',
...
fr:{
  ...
  'co-how-t':'Le mettre en place — trois étapes',
  'co-how-1':'<strong style="color:var(--text)">Démarrez-le.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;votre-hôte&gt;</code> génère les deux clés sur cette machine avec les droits 600, démarre le conteneur et affiche votre clé publique. Rien à copier au préalable, rien à envoyer où que ce soit.',
  'co-how-2':'<strong style="color:var(--text)">Signez l\'autorisation.</strong> Collez cette clé publique dans le bouton ci-dessous et signez avec un portefeuille qui est un humain enregistré. Rien n\'est dépensé ni transféré : on signe une phrase.',
  'co-how-3':'<strong style="color:var(--text)">Envoyez-la aux nœuds.</strong> La page vous remet une ligne se terminant par <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;signature&gt;</code>. Exécutez-la sur le serveur de votre coordinateur. <strong style="color:var(--text)">Signer seul ne change rien</strong> — c\'est cette commande qui inscrit l\'autorisation, et elle part vers tous les nœuds à la fois.',
...
pt:{
  ...
  'co-how-t':'Pôr a funcionar — três passos',
  'co-how-1':'<strong style="color:var(--text)">Arranque-o.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;o-seu-hostname&gt;</code> gera ambas as chaves nessa máquina com permissões 600, arranca o contentor e imprime a sua chave pública. Nada para copiar antes, nada para enviar seja para onde for.',
  'co-how-2':'<strong style="color:var(--text)">Assine a autorização.</strong> Cole essa chave pública no botão abaixo e assine com uma carteira que seja uma pessoa registada. Nada é gasto nem transferido — assina-se uma frase.',
  'co-how-3':'<strong style="color:var(--text)">Envie-a aos nós.</strong> A página entrega-lhe uma linha terminada em <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;assinatura&gt;</code>. Execute-a no servidor do seu coordenador. <strong style="color:var(--text)">Assinar por si só não muda nada</strong> — é este comando que escreve a autorização, e vai a todos os nós de uma vez.',
...
ar:{
  ...
  'co-how-t':'الإعداد — ثلاث خطوات',
  'co-how-1':'<strong style="color:var(--text)">شغّله.</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;اسم-مضيفك&gt;</code> يولّد المفتاحين على تلك الآلة بصلاحيات 600، ويشغّل الحاوية، ويطبع مفتاحك العام. لا شيء يُنسخ مسبقًا، ولا شيء يُرسل إلى أي مكان.',
  'co-how-2':'<strong style="color:var(--text)">وقّع التفويض.</strong> ألصق ذلك المفتاح العام في الزر أدناه ووقّع بمحفظة تعود إلى إنسان مسجَّل. لا يُنفق شيء ولا يُحوَّل شيء — التوقيع على جملة.',
  'co-how-3':'<strong style="color:var(--text)">أرسله إلى العقد.</strong> تعطيك الصفحة سطرًا ينتهي بـ <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;التوقيع&gt;</code>. نفّذه على خادم منسّقك. <strong style="color:var(--text)">التوقيع وحده لا يغيّر شيئًا</strong> — هذا الأمر هو ما يكتب التفويض، وهو يذهب إلى كل العقد دفعة واحدة.',
...
hi:{
  ...
  'co-how-t':'स्थापना — तीन चरण',
  'co-how-1':'<strong style="color:var(--text)">इसे शुरू करें।</strong> <code style="color:var(--teal)">./eigenen-coordinator-starten.sh &lt;आपका-होस्टनेम&gt;</code> उसी मशीन पर 600 अनुमतियों के साथ दोनों कुंजियाँ बनाता है, कंटेनर चलाता है, और आपकी सार्वजनिक कुंजी छापता है। पहले से कुछ कॉपी करने की ज़रूरत नहीं, कहीं कुछ भेजने की भी नहीं।',
  'co-how-2':'<strong style="color:var(--text)">अधिकार-पत्र पर हस्ताक्षर करें।</strong> उस सार्वजनिक कुंजी को नीचे के बटन में चिपकाएँ और ऐसे बटुए से हस्ताक्षर करें जो पंजीकृत मनुष्य हो। न कुछ ख़र्च होता है न कुछ हस्तांतरित — हस्ताक्षर एक वाक्य पर होता है।',
  'co-how-3':'<strong style="color:var(--text)">इसे नोड्स को भेजें।</strong> पृष्ठ आपको एक पंक्ति देता है जो <code style="color:var(--teal)">./coordinator-eintragen.sh 0x&lt;हस्ताक्षर&gt;</code> पर समाप्त होती है। उसे अपने समन्वयक के होस्ट पर चलाएँ। <strong style="color:var(--text)">केवल हस्ताक्षर करने से कुछ नहीं बदलता</strong> — अधिकार यही आदेश लिखता है, और वह एक साथ सभी नोड्स तक जाता है।',
