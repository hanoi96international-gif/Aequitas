...
de:{
  ...
  'bv-guide-lang-note':'Die Schritte unten sind auf Englisch. Die Abschnitte darüber — worum es geht, was geschützt wird und wofür du einstehst — stehen in deiner Sprache.',
  'bv-what-d':'Ein Gesicht erreicht dich nie als Bild: die Aufnahme wird nach der Prüfung verworfen, und deine Maschine speichert eine verschlüsselte Vorlage davon. Es gibt einen Modus mit geteilten Anteilen, in dem deine Maschine nur einen additiven Anteil einer 64-Byte-Skizze hielte — für sich genommen von Rauschen nicht zu unterscheiden, gemeinsam mit deinem Komitee verglichen, sodass keiner mehr erfährt als die Antwort. Er ist gebaut und geprüft, aber <strong style="color:var(--text)">noch nicht scharf</strong>, weil seine Schwelle nie kalibriert wurde. Bis dahin hält jeder Verifier eine ganze verschlüsselte Vorlage — und das sagen wir lieber, als einen Schutz anzudeuten, den du noch nicht gibst.',
  ...
  'reg-sub':'Tritt dem Aequitas-Netz bei und erhalte deinen 1.000-AEQ-Startzuschuss. Die Registrierung ist einmalig und dauerhaft. Phase 1: eine Live-Gesichtsprüfung durch unabhängige Vergleichsdienste ist das Tor — die benannten Grenzen stehen im Disclaimer.',
  'app-title':'REGISTRIERUNG NUR ÜBER ANDROID-APP',
  'app-text':'Die Registrierung nimmt dein Gesicht und eine kurze Lebendigkeitssequenz (zufällige Kopfdrehung) mit der Handykamera auf. Zwei unabhängige Vergleichsdienste prüfen, dass ein lebender Mensch vor der Kamera ist und dieses Gesicht nicht schon registriert ist; sie müssen sich einig sein, und der Proof-Server nimmt die Registrierung nur mit ihrer signierten Bescheinigung an. Ein Groth16-ZK-Beweis trägt das Ergebnis dann auf die Chain, ohne etwas über dich preiszugeben. Deine <strong style="color:var(--gold)">1.000 AEQ werden automatisch gutgeschrieben</strong>, sobald die Prüfung durch ist. <strong style="color:var(--gold)">Hinweis:</strong> die benannten Grenzen (Konten von vor dem 25.08.2026, unkalibrierte Schwelle, Kopfdreh-Lebendigkeit) stehen im Disclaimer neben dem Registrieren-Knopf.',
  's1t':'Verbinden','s1d':'Die App nimmt dein Gesicht und eine kurze Lebendigkeitssequenz (zufällige Kopfdrehung) auf und schickt sie an unabhängige Vergleichsdienste. Die prüfen, dass ein lebender Mensch vor der Kamera ist, und vergleichen das Gesicht mit allen seit Beginn der Gesichtsprüfung Registrierten. Die Bilder werden nach der Verarbeitung verworfen.',
  's2t':'Beweisen','s2d':'Ein Groth16-ZK-Beweis bindet deine bio_hash in commitment = keccak256(bioHash‖wallet), ohne sie preiszugeben. Der Proof-Server stellt ihn nur gegen die signierte Bescheinigung der Vergleichsdienste aus, und der daraus abgeleitete Nullifier lässt sich genau einmal verbrauchen — dasselbe Gesicht zählt also nicht zweimal.',
  ...
  'swap-sub':'Tausche AEQ gegen tUSD (ein simulierter Test-Dollar) über den nativen Liquiditäts-Pool. 0,1% Gebühr für Swaps; gewöhnliche AEQ-Transfers tragen ebenfalls 0,1 % Protokollgebühr (kein EVM-Gas).',
  ...
  'faq-q2':'Kann ich mich später mit einer anderen Wallet registrieren?','faq-a2':'Nein. Eine Registrierung ist dauerhaft an eine Wallet-Adresse gebunden. Das ist Absicht: der aus deinem Gesicht abgeleitete Nullifier wird genau einmal verbraucht; eine erneute Registrierung auf eine andere Wallet wäre eine zweite Identität für dieselbe Person.',
