...
// Disabled by default. A consensus change that switched itself on as soon as
// the binary shipped would fork the network against every node still rolling
// out.
func TestUBIChunking_DisabledUnlessAnActivationHeightIsSet(t *testing.T) {
	...
	if UBIChunkingActive(1_999_999) {
		t.Fatal("must not activate below the configured height — nodes would disagree about which blocks carry which credits")
	}
	if !UBIChunkingActive(2_000_000) {
		t.Fatal("must activate exactly at the configured height")
	}
}
