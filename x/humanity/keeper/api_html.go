...
<div class="rs">
  ...
  <div class="rsteps">
    ...
    <div class="rstep"><div class="snum">4</div><div class="stitle" data-i18n="s4t">1,000 AEQ Granted</div><div class="sdesc" data-i18n="s4d">Your registration is confirmed on the Aequitas BlockDAG within 1 second. 1,000 AEQ is credited to your wallet instantly, gasless. Your identity is permanently recorded as verified human.</div></div>
  </div>
  ...
  <div class="ic">
    ...
    <div class="ic-row"><span class="ic-key" data-i18n="k-conf">Confirmation</span><span class="ic-val" data-i18n="k-conf-v">Within 1 second (1 block)</span></div>
...
<div class="nc" style="grid-column:1/-1;background:linear-gradient(135deg,rgba(245,166,35,0.06),rgba(13,8,32,0.9));border-color:rgba(245,166,35,0.2)">
    ...
    <div id="node-guide" style="display:block;background:var(--card);border:1px solid rgba(245,166,35,0.2);border-radius:var(--radius);padding:24px;margin-top:4px">
      ...
      <div style="background:rgba(0,220,170,0.05);border:1px solid rgba(0,220,170,0.15);border-radius:6px;padding:10px 14px;margin-bottom:10px;font-size:0.6rem"><span style="color:var(--teal);font-weight:700">What is a database?</span> <span style="color:var(--muted)">Your node needs permanent storage for all block data and human registrations. Without it, your node loses all data on every restart. PostgreSQL is the storage system Aequitas uses. Each node must have its <strong style="color:var(--text)">own dedicated database</strong> — never share one database between two nodes.</span></div>

      <div style="font-size:0.62rem;color:var(--purple);font-weight:700;margin-bottom:6px;margin-top:4px">Option A &mdash; Railway (easiest, recommended for beginners)</div>
      <div style="font-size:0.62rem;color:var(--muted);line-height:2.2;margin-bottom:14px">
        ...
        <div><span style="color:var(--gold);font-weight:700">d)</span> Railway creates and manages your database automatically. DATABASE_URL is injected into your node service for free &mdash; no manual configuration needed.</div>
      </div>

      <div style="font-size:0.62rem;color:var(--purple);font-weight:700;margin-bottom:6px">Option B &mdash; VPS / own server (Contabo, Hetzner, DigitalOcean)</div>
      <div style="background:rgba(245,166,35,0.05);border:1px solid rgba(245,166,35,0.2);border-radius:6px;padding:8px 12px;margin-bottom:8px;font-size:0.6rem;color:var(--muted)"><strong style="color:var(--gold)">Important:</strong> Install PostgreSQL directly on your VPS — do <strong>not</strong> connect to Railway's PostgreSQL from a different VPS. Both nodes would share one database → state conflicts and your data could be overwritten.</div>
      <div style="font-family:var(--font-mono);background:rgba(0,0,0,0.3);border:1px solid rgba(139,92,246,0.15);border-radius:6px;padding:14px;margin-bottom:8px;font-size:0.61rem;line-height:2.3;overflow-x:auto">
        <span style="color:var(--muted)"># 1. Install PostgreSQL (Ubuntu / Debian)</span><br>
        sudo apt update &amp;&amp; sudo apt install -y postgresql postgresql-contrib<br>
        sudo systemctl enable --now postgresql<br><br>
        <span style="color:var(--muted)"># 2. Create database and user (run as root or with sudo)</span><br>
        sudo -u postgres psql -c <span style="color:var(--gold)">"CREATE USER aequitas WITH PASSWORD 'CHOOSE_A_STRONG_PASSWORD';"</span><br>
        sudo -u postgres psql -c <span style="color:var(--gold)">"CREATE DATABASE aequitas OWNER aequitas;"</span><br><br>
        <span style="color:var(--muted)"># 3. Allow connections from Docker containers (Docker bridge: 172.17.0.0/16)</span><br>
        PG_CONF=$(sudo -u postgres psql -t -c <span style="color:var(--gold)">"SHOW hba_file;"</span> | tr -d ' ')<br>
        echo <span style="color:var(--gold)">"host aequitas aequitas 172.17.0.0/16 md5"</span> | sudo tee -a $PG_CONF<br>
        sudo -u postgres psql -c <span style="color:var(--gold)">"ALTER SYSTEM SET listen_addresses = '*';"</span><br>
        sudo systemctl restart postgresql<br><br>
        <span style="color:var(--muted)"># 4. Test (should print one row)</span><br>
        psql <span style="color:var(--gold)">"postgres://aequitas:CHOOSE_A_STRONG_PASSWORD@localhost:5432/aequitas"</span> -c <span style="color:var(--gold)">"SELECT 1;"</span><br><br>
        <span style="color:var(--muted)"># Your DATABASE_URL for the docker run command below:</span><br>
        <span style="color:var(--teal)"># postgres://aequitas:CHOOSE_A_STRONG_PASSWORD@172.17.0.1:5432/aequitas</span><br>
        <span style="color:var(--muted)"># (172.17.0.1 = Docker bridge gateway, how containers reach the VPS host)</span>
      </div>
      <div style="background:rgba(0,220,170,0.05);border:1px solid rgba(0,220,170,0.15);border-radius:6px;padding:8px 12px;margin-bottom:18px;font-size:0.6rem;color:var(--muted)"><strong style="color:var(--neon)">Firewall:</strong> PostgreSQL only needs to be reachable from within the VPS itself. Do not open port 5432 to the internet. <code style="color:var(--teal)">ufw deny 5432</code> (or leave it closed — most VPS firewalls block it by default).</div>
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:18px">
        ...
        <tr style="border-bottom:1px solid rgba(139,92,246,0.08);background:rgba(0,0,0,0.1)">
          ...
          <td style="font-size:0.6rem;color:#f87171;padding:8px;font-weight:700">YES</td>
          <td style="font-size:0.61rem;color:var(--muted);padding:8px">Your node's own public URL (e.g. <span style="font-family:var(--font-mono);color:var(--neon)">https://my-node.up.railway.app</span> or <span style="font-family:var(--font-mono);color:var(--neon)">http://YOUR-IP:8080</span> on VPS). Without it the node starts in isolated mode — it cannot register with peers, cannot propagate blocks, and other nodes cannot reach it. On Railway: Settings &rarr; Networking &rarr; Public Networking.</td>
        </tr>
        <tr style="border-bottom:1px solid rgba(139,92,246,0.08)">
        ...
        <tr style="border-bottom:1px solid rgba(139,92,246,0.08);background:rgba(0,0,0,0.1)">
          ...
          <td style="font-size:0.6rem;color:var(--muted);padding:8px">No</td>
          <td style="font-size:0.61rem;color:var(--muted);padding:8px">Removed — does nothing. Leave unset.</td>
        </tr>
        <tr style="border-bottom:1px solid rgba(139,92,246,0.08)">
          <td style="font-size:0.61rem;font-family:var(--font-mono);color:var(--neon);padding:8px">DISTRIBUTION_ENABLED</td>
          <td style="font-size:0.6rem;color:var(--muted);padding:8px">No</td>
          <td style="font-size:0.61rem;color:var(--muted);padding:8px">Leave unset for normal operation — all nodes are eligible to trigger the daily pool distribution by default. Three-layer dedup (cross-node <span style="font-family:var(--font-mono)">last_ubi_at</span> check + intra-node CAS lock + replay pre-pass) prevents double-credit even when multiple nodes fire at the same time. Set to <span style="font-family:var(--font-mono);color:#f87171">false</span> only if you explicitly want to opt this node out of running distributions (e.g. a resource-constrained replica).</td>
        </tr>
        <tr style="border-bottom:1px solid rgba(139,92,246,0.08)">
      ...
      <!-- Step 4b Docker -->
      <div style="font-size:0.58rem;color:var(--purple);font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:10px;border-bottom:1px solid var(--border);padding-bottom:6px">Step 4b &mdash; Alternative: Deploy on a VPS with Docker</div>
      <div style="font-size:0.62rem;color:var(--muted);line-height:1.9;margin-bottom:8px">For your own server (Contabo, Hetzner, DigitalOcean). Requires Docker installed on the VPS and a local PostgreSQL database set up via Step 2 Option B above. <strong style="color:var(--gold)">Do not connect to Railway's PostgreSQL from a VPS</strong> — each node must have its own separate database. NODE_OPERATOR_WALLET must be a registered Aequitas human wallet.</div>
      <div style="font-family:var(--font-mono);background:rgba(0,0,0,0.3);border:1px solid rgba(139,92,246,0.15);border-radius:6px;padding:14px;margin-bottom:8px;font-size:0.61rem;line-height:2.2;overflow-x:auto">
        <span style="color:var(--muted)"># 1. Install Docker (if not already installed)</span><br>
        curl -fsSL https://get.docker.com | sh<br><br>
        <span style="color:var(--muted)"># 2. Clone and build the node (~3 min Go compile)</span><br>
        git clone https://github.com/hanoi96international-gif/Aequitas &amp;&amp; cd Aequitas<br>
        docker build -t aequitas-node .<br><br>
        <span style="color:var(--muted)"># 3. First start (NODE_KEY will be printed in logs — see step 4)</span><br>
        <span style="color:var(--muted)">#    DATABASE_URL uses 172.17.0.1 = Docker bridge gateway → host PostgreSQL</span><br>
        <span style="color:var(--muted)">#    (set up PostgreSQL first via Step 2 Option B above)</span><br>
        docker run -d --name aequitas-node --restart unless-stopped \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">DATABASE_URL</span>="<span style="color:var(--gold)">postgres://aequitas:YOUR_DB_PASSWORD@172.17.0.1:5432/aequitas</span>" \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">RELAYER_PRIVATE_KEY</span>="<span style="color:var(--gold)">0xYOUR_PRIVATE_KEY</span>" \<br>
        ...
        <span style="color:var(--muted)"># 5. Stop, add NODE_KEY, restart permanently:</span><br>
        ...
        &nbsp;&nbsp;-e <span style="color:var(--neon)">NODE_OPERATOR_WALLET</span>="<span style="color:var(--gold)">0xYOUR_REGISTERED_HUMAN_WALLET</span>" \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">NODE_OPERATOR_BINDING_SIGNATURE</span>="<span style="color:var(--gold)">generate-at-/node-binding</span>" \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">NODE_KEY</span>="<span style="color:var(--gold)">base64-from-step-4</span>" \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">SELF_URL</span>="<span style="color:var(--gold)">http://YOUR-SERVER-IP:8080</span>" \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">PRIMARY_NODE_URL</span>="https://aequitas.digital" \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">BOOTSTRAP_SNAPSHOT_URL</span>="https://aequitas.digital/api/snapshot" \<br>
        &nbsp;&nbsp;-e <span style="color:var(--neon)">BOOTSTRAP_SIGNER</span>="0x92cbedec9d348b4762cb9af99500ee6139c5b671" \<br>
...
ru:{
  ...
  'recent-blocks':'Последние Блоки','blocks-desc':'MERGE = объединение нескольких родителей (BlockDAG). TX = транзакция регистрации. Время блока: ~1 секунда.',
