...
- **`IS_PRIMARY_NODE`** ist auf keiner Box gesetzt. Betrieblich unkritisch
  ...
  Voreinstellung.
- **`git_commit: unknown`** in `/api/status` auf beiden Boxen. Das Feld selbst
  funktioniert (`readBuildGitCommit`, `version_test.go`) — `"unknown"` ist
  sein dokumentierter Fallback, wenn `debug.ReadBuildInfo()` keine VCS-Daten
  liefert. Genau das ist im Docker-Build der Fall: das `Dockerfile` kopiert
  kein `.git`, also kann Go die Commit-SHA nicht einbetten. Folge: im Betrieb
  ist nicht ablesbar, welcher Stand läuft. Sauber lösbar über ein
  Build-Argument mit `-ldflags`, das der Workflow aus `github.sha` füllt.

---

## 6. Branch-Lage (wichtig, beim Audit aufgefallen)

`master` liegt **20+ Commits hinter `origin/main`** und hat nichts Exklusives
— es ist schlicht ein veralteter Stand. Die gesamte 50k-TPS-Arbeit (#89–#109)
liegt auf `main`.

**Produktion läuft auf `main`.** Belegt über die Deploy-Historie: der letzte
erfolgreiche Lauf beider Workflows war am **2026-07-29 19:47** für Commit
`6f82ee2` (#109) — das deckt sich mit den gemessenen Uptimes (C1 ≈ 10,8 Tage,
C2 ≈ 15,9 Tage) und heißt zugleich: seit 16 Tagen kein Deploy.

Konsequenzen:
- Arbeit gehört auf `main`, nicht auf `master`. Dieser Branch basiert auf
  `6f82ee2`.
- Zwei Befunde aus dem ersten Audit-Durchgang betrafen **nur `master`** und
  sind auf `main` längst behoben: die Google-Fonts-Einbindung in `landing.go`
  (dort bereits `fonts.bunny.net`) und das fehlende `git_commit`-Feld.
- Die uncommittete Audit-Arbeit vom 2026-07-21 (CSP-Header, RPC-Batch-Rate-
  Limit, XSS-Fix in `node-binding.js`, zwei Solidity-Härtungen) liegt im
  `master`-Working-Tree und ist auf `main` **nicht** gemerged. Sie ist damit
  nie in Produktion gelangt.
- Die beiden Solidity-Änderungen brauchen ein eigenes, geplantes V7-Upgrade:
  ein Redeploy wird über `V7ContractVersion` ausgelöst
  (`contract_deploy.go`) und wipet dabei EVM-Storage mit anschließender
  Migration. Das gehört nicht nebenbei in ein Audit.
